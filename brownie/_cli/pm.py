...
def _delete(id_):
    path = _get_data_folder().joinpath(f"packages/{id_}")
    shutil.rmtree(path)
...
def _install_from_github(package_id):
    try:
        ...
        print("Invalid package ID. Must be given as [ORG]/[REPO]@[VERSION]")
        print("e.g. openzeppelin/openzeppelin-contracts@v2.5.0")
        return
